...
# Add CLI commands to the Flask app
...
class Product(db.Model):
    ...
    image_category = db.Column(db.String(80), nullable=False)  # New field for image category

    def __repr__(self):
        return f'<Product {self.name}>'

# RelatedImages model to link images to categories
class RelatedImage(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    product_id = db.Column(db.Integer, db.ForeignKey('product.id'), nullable=False)
    image_url = db.Column(db.String(200), nullable=False)
    product = db.relationship('Product', backref='related_images')
